...
def load_data(molecules, device, path = 'data', load_obs_mat = True, 
    ...
    for molecule in molecules:
        ...
        if(ind_list == []):
            ...
            nuclearCharge = torch.tensor(nuclearCharge, dtype=torch.float64).to(device);
...
def load_data_pyscf(molecules, device, paras, load_obs_mat = True, 
              ind_list = [], op_names = []):
    ...
    for molecule in molecules:
        #with open(path+'/'+molecule+'_data.json', 'r') as file:
            ...
            #data = json.load(file);
        data = {}
        data['elements'] = paras['elements']
        data['coordinates'] = paras['coordinates']
        data['h'] = paras['h']
        data['S'] = paras['S']
        data['Enn'] = paras['Enn']
        data['energy'] = paras['energy']
        ...
        if(ind_list == []):
            ...
            h = torch.tensor(data['h'],dtype=torch.float64)[ind_list].to(device);
            S_mhalf = [scipy.linalg.fractional_matrix_power(data['S'][si], (-1/2)).tolist() for si in ind_list]
            S_mhalf = torch.tensor(np.array(S_mhalf).real,dtype=torch.float64).to(device);
            h = torch.matmul(torch.matmul(S_mhalf, h),S_mhalf);
