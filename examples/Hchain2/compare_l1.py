...
def grep_opt_l1(xyzfile='Hchain.xyz', opt_log_file="opt_log.txt", res_file='res_E.json'):
    ...
    for line in reversed(lines):
        if "l-1:" in line:
            loss_value = line.split("l-1:")[-1].split()[0].strip()
    ...
    res_data['opt_l1'] = loss_value

    with open(res_file, 'w') as f:
        json.dump(res_data, f, indent=4)
    
    end_time = time.time()
    print(f"Successfully updated {res_file} with opt_l1: {loss_value}")
    print(f"Execution time: {end_time - start_time:.4f} seconds")

def grep_init_l1(xyzfile='Hchain.xyz', opt_log_file="opt_log.txt", res_file='res_E.json'):
    start_time = time.time()
...
def main():
    ...
    for i in range(args.start, args.end + 1):
        ...
        grep_opt_l1(xyzfile=xyzfile, opt_log_file=opt_log_file,res_file=res_file)
        grep_init_l1(xyzfile=xyzfile, opt_log_file=opt_log_file,res_file=res_file)
