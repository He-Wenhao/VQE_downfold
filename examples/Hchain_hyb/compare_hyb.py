...
def df_energy(xyzfile='Hchain.xyz', opt_log_file="opt_log.txt", res_file='res_E.json', method='HF',basisFile=None,saveHam=False,ham_file='ham.json'):
    ...
    if saveHam == True:
        try:
            ...
            ham_data = {}
        ham_data[name] = [ham.Ham_const,ham.int_1bd.tolist(),ham.int_2bd.tolist()]
...
def sto3G_energy(xyzfile='Hchain.xyz', opt_log_file="opt_log.txt", res_file='res_E.json',saveHam=False,ham_file='ham.json'):
    ...
    if saveHam == True:
        try:
            ...
            ham_data = {}
        ham_data['sto-3G'] = [ham.Ham_const,ham.int_1bd.tolist(),ham.int_2bd.tolist()]
        with open(ham_file, 'w') as f:
            json.dump(ham_data, f, indent=4)
...
def main():
    ...
    for i in range(args.start, args.end + 1):
        ...
        ham_file = f"H_chain_xyzs/{args.atoms}/{i}/ham.json"
        # grep_opt_energy(xyzfile=xyzfile, opt_log_file=opt_log_file,res_file=res_file)
        for method in ['HF','B3LYP']:
            df_energy(xyzfile=xyzfile, opt_log_file=opt_log_file,res_file=res_file,method = method,saveHam=args.saveHam,ham_file=ham_file)
